...
public class AiAgentController {
    ...
    private static final String SYSTEM_PROMPT_TEMPLATE = """
            ...
            - 사용자가 검색 결과를 원하는 모든 경우(예: '검색', '찾아줘', '보여줘', '알려줘', 또는 지역·기간·가격 등 조건만 말한 경우)에는 기본적으로 action=search를 사용한다. setFilters는 '검색하지 말고 조건만 설정'처럼 검색 미실행을 사용자가 명시적으로 요청할 때만 사용한다.
            - filters 맵에 사용할 수 있는 키는 정확히 다음뿐이다: %s
              목록에 없는 키는 절대 만들지 마라.
            - 값은 모두 문자열로 적는다. 거래월은 'YYYY-MM' 형식(예: '2024-05'), 자치구는 '강남구'처럼 '구'를 포함한다.
            - 거래월은 절대 clarify 사유로 삼지 마라. 사용자가 어떤 연월(2025년·2026년 등 포함)을 말하든 그대로 startDealMonth/endDealMonth에 넣고 action=search로 처리하라. 거래월의 과거/미래/지원 여부 판단과 범위 검증(2006년~현재)은 전적으로 시스템이 수행한다. 네가 연도 상한('2024년까지' 등)을 추정하거나 거래월을 이유로 되묻는 것을 금지한다.
